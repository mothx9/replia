 ...
 * A neutral C host loop. Build using only an installed replai.h and library.
...
static int tracing;
static int check(replai_status status) {
    if (status == REPLAI_OK) return 1;
    uint8_t text[128];
    size_t n = 0;
    if (replai_status_text(status, text, sizeof text, &n) == REPLAI_OK)
        fprintf(stderr, "error %d: %.*s\n", (int)status, (int)n, (const char *)text);
    else fprintf(stderr, "error %d\n", (int)status);
    return 0;
}
static uint8_t *draft(replai_handle *h, size_t *n, size_t *cursor) {
    if (!check(replai_draft_copy(h, NULL, 0, n, cursor))) return NULL;
    uint8_t *text = malloc(*n + 1);
    if (!text) return NULL;
    if (!check(replai_draft_copy(h, text, *n, n, cursor))) { free(text); return NULL; }
    text[*n] = 0; /* Only this C-owned convenience buffer adds a terminator. */
    return text;
}
static int trace(replai_handle *h, const char *label, uint32_t kind, int32_t status) {
    if (!tracing) return 1;
    size_t n, cursor;
...
int main(int argc, char **argv) {
    ...
    uint32_t version = 0;
    if (!check(replai_abi_version(&version)) || version != REPLAI_C_ABI_VERSION) return 1;
    ...
    if (!check(replai_create(&config, &h))) goto done;
    if (!check(replai_prompt(h, (const uint8_t *)"demo", 4, NULL, 0,
                            (const uint8_t *)"... ", 4))) goto done;
    for (;;) {
        if (fflush(stdout) != 0 || !check(replai_open(h, STDIN_FILENO, STDOUT_FILENO))) goto done;
        ...
        if (palette) {
            ...
            for (uint32_t role = 0; role < 7; role++)
                if (!check(replai_external_output(h, role, (const uint8_t *)labels[role], strlen(labels[role])))) goto done;
        ...
        for (;;) {
            if (notice && !notice_sent && now() - started >= 1.0) {
                ...
                if (!check(replai_external_output(h, REPLAI_ROLE_DIM, message, sizeof message - 1))) goto done;
            ...
            if (!check(replai_poll(h, 100, &event))) goto done;
            if (event.kind == REPLAI_EVENT_NONE) continue;
            if (!trace(h, "EVENT", event.kind, event.status)) goto done;
            if (event.kind == REPLAI_EVENT_COMPLETION_REQUESTED) {
                ...
                free(text);
                if (matches == 1 && !check(replai_complete(h, 0, n, (const uint8_t *)selected, strlen(selected)))) goto done;
                if (!trace(h, "COMPLETE", event.kind, 0)) goto done;
            } else if (event.kind == REPLAI_EVENT_EDIT_REJECTED) {
                const uint8_t message[] = "input rejected";
                if (!check(replai_external_output(h, REPLAI_ROLE_WARNING, message, sizeof message - 1))) goto done;
            } else break;
        }
        /* These outcomes already restored and released terminal ownership. */
        if (!check(replai_close(h))) goto done;
        if (event.kind == REPLAI_EVENT_SUBMITTED) {
            size_t n = 0;
            if (!check(replai_submitted_copy(h, NULL, 0, &n))) goto done;
            uint8_t *text = malloc(n ? n : 1);
            if (!text) goto done;
            int copied = check(replai_submitted_copy(h, text, n, &n));
            int admitted = copied && (!n || check(replai_history_add(h, text, n)));
            int printed = 0;
            if (admitted) printed = fputs("echo: ", stdout) >= 0 && fwrite(text, 1, n, stdout) == n && fputs("\n\n", stdout) >= 0;
            free(text);
            if (!admitted || !printed) goto done;
        }
        if (once || event.kind == REPLAI_EVENT_END_OF_INPUT) break;
        if (!check(replai_clear(h))) goto done;
    }
    exit_status = 0;
done:
    if (h && !check(replai_destroy(&h))) exit_status = 1;
